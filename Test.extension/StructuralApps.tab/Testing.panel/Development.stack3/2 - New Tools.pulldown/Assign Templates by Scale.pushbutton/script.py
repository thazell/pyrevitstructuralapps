...
print "Section view list:" + str(len(sectionviewlist))
print "Drafting view list:" + str(len(draftingviewlist))
...
sectiontemplatelist.sort(key=lambda x: x.Id.IntegerValue)
...
sheetedsectionviews = filter(lambda x: (x.ViewTemplateId == ElementId.InvalidElementId and x.get_Parameter(BuiltInParameter.VIEWPORT_SHEET_NUMBER).HasValue),sectionviewlist)
sheeteddraftingviews = filter(lambda x: (x.ViewTemplateId == ElementId.InvalidElementId and x.get_Parameter(BuiltInParameter.VIEWPORT_SHEET_NUMBER).HasValue),draftingviewlist)
...
for v in sheeteddraftingviews:
    vscale = v.get_Parameter(BuiltInParameter.VIEW_SCALE).AsInteger()
    for t in draftingtemplatelist:
        ...
        # print v.Name
        # print vscale
        if t.get_Parameter(BuiltInParameter.VIEW_SCALE).AsInteger() == vscale:
...
for v in sheetedsectionviews:
    ...
    for t in sectiontemplatelist:
      ...
      continue
if ii == 0: print "None"
if ii !=0: print "Total Details/Sections Modified: " + str(ii)
...
mainDialog.AddCommandLink(TaskDialogCommandLinkId.CommandLink1,"Undo changes?")
mainDialog.AddCommandLink(TaskDialogCommandLinkId.CommandLink2,"Keep Changes")
