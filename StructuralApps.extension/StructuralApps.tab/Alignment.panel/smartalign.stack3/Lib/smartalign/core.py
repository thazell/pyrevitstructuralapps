...
def get_location(element, align_method):
    ...
    try:  # Try Coord - For Text Elements
        ...
        logger.debug('1Could not get .Coord.')
        logger.debug('Error: {}'.format(errmsg))
    else:
        location_pt = PointElement(location_pt.X, location_pt.Y, location_pt.Z)
        logger.debug('1Got Location from .Location: {}'.format(location_pt))
        return location_pt

    try:  # Try .Location Method First - Needed for Rooms
        location_pt = element.Location.Point
    except Exception as errmsg:
        logger.debug('2Could not get .Location. Trying bbox...')
        logger.debug('Error: {}'.format(errmsg))
    else:
        location_pt = PointElement(location_pt.X, location_pt.Y, location_pt.Z)
        logger.debug('2Got Location from .Location: {}'.format(location_pt))
        return location_pt

    try:  # Try TAG HEAD  For Text Elements
        location_pt = element.TagHeadPosition
    except Exception as errmsg:
        logger.debug('Could not get Tag Head Position.')
        logger.debug('Error: {}'.format(errmsg))
    else:
        location_pt = PointElement(location_pt.X, location_pt.Y, location_pt.Z)
        logger.debug('0Got Location from .Tag Head Position: {}'.format(location_pt))
        return location_pt

    try:  # Not Room: Try Bounding Box Method
        bbox = BoundingBoxElement(element)
    except Exception as errmsg:
        logger.debug('4Could not get BBox for:{}'.format(type(element)))
        logger.debug('Error: {}'.format(errmsg))
    else:
        location_pt = getattr(bbox, align_method)  # min, max, or average
        logger.debug('3Got Location by Bounding Box: {}'.format(location_pt))
        return location_pt
