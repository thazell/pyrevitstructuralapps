...
__doc__ = 'Selects elements in the current view that do not have tags.'

__window__.Close()

import sys
from Autodesk.Revit.DB import FilteredElementCollector, BuiltInCategory, ElementId, ViewSheet, Structure
...
commandSwitches(['Structural Columns',
                 'Structural Framing',
...
if selected_switch == 'Rooms':
...
elif selected_switch == 'Doors' or selected_switch == 'Windows' or selected_switch == 'Walls' or selected_switch == 'Structural Framing' or selected_switch == 'Structural Columns' or selected_switch == 'Structural Foundations' or selected_switch == 'Floors':
    if selected_switch == 'Doors':
        ...
        elname = 'Walls'
    elif selected_switch == 'Structural Framing':
		tagcat = BuiltInCategory.OST_StructuralFramingTags
		elcat = BuiltInCategory.OST_StructuralFraming
		elname = 'Structural Framing'
    elif selected_switch == 'Structural Columns':
		tagcat = BuiltInCategory.OST_StructuralColumnTags
		elcat = BuiltInCategory.OST_StructuralColumns
		elname = 'Structural Columns'
    elif selected_switch == 'Structural Foundations':
		tagcat = BuiltInCategory.OST_StructuralFoundationTags
		elcat = BuiltInCategory.OST_StructuralFoundation
		elname = 'Structural Foundations'
    elif selected_switch == 'Floors':
		tagcat = BuiltInCategory.OST_FloorTags
		elcat = BuiltInCategory.OST_Floors
		elname = 'Floors'
 ...
 #   TaskDialog.Show('test', str(len(elementtags)))
 #   TaskDialog.Show('test', str(len(elements)))
    ...
    for elid in elements:
        ...
        #try:
        #    typecomment = el.Symbol.LookupParameter('Type Mark')
        ...
        try:
            if el.Id.IntegerValue not in tagged_elements and el.StructuralType != Structure.StructuralType.Brace:
                untagged_elements.append(elid)
        except:
    ...
    if len(untagged_elements) > 0:
        TaskDialog.Show('Untagged Element Selection.', str(len(untagged_elements)) + ' {} have been selected'.format(elname).lower())
